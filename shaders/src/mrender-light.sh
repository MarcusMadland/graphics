...
vec4 calcLight(vec2 texcoord0)
{
    ...
    float shadowFactor = currentDepth > closestDepth  ? 0.2 : 1.0;  
	if(projCoords.z > 1.0)
	{
		shadowFactor = 1.0;
	}
	...
	vec3 posOffset = mul(wpos, shadowMapOffset);
	vec4 shadowCoords = mul(u_lightMtx, vec4(posOffset, 1.0) );
	//shadowFactor = PCF(shadowCoords, shadowMapBias, texelSize);
	
	// Return final color
	return mul(vec4(lighting.rgb, 1.0), shadowFactor);
	//return vec4(shadowFactor, 0.0, 0.0, 1.0);
	//return vec4(lighting.rgb, 1.0);
