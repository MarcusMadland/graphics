...
#ifndef GRAPHICS_H_HEADER_GUARD
#define GRAPHICS_H_HEADER_GUARD
...
#define GRAPHICS_HANDLE(_name)                                                           \
	...
	inline bool isValid(_name _handle) { return graphics::kInvalidHandle != _handle.idx; }

#define GRAPHICS_INVALID_HANDLE { graphics::kInvalidHandle }

namespace base { struct AllocatorI; }
...
namespace graphics
{
	/// Fatal error enum.
	///
	/// @attention C99's equivalent binding is `graphics_fatal_t`.
	///
	struct Fatal
	{
		enum Enum
	...
	///
	/// @attention C99's equivalent binding is `graphics_renderer_type_t`.
	///
	struct RendererType
	{
		/// Renderer types:
	...
	/// Access mode enum.
	///
	/// @attention C99's equivalent binding is `graphics_access_t`.
	///
	struct Access
	{
		/// Access:
	...
	///
	/// @attention C99's equivalent binding is `graphics_attrib_t`.
	///
	struct Attrib
	{
	...
	///
	/// @attention C99's equivalent binding is `graphics_attrib_type_t`.
	///
	struct AttribType
	{
		/// Attribute types:
		...
		{
			Uint8,  //!< Uint8
			Uint10, //!< Uint10, availability depends on: `GRAPHICS_CAPS_VERTEX_ATTRIB_UINT10`.
			Int16,  //!< Int16
			Half,   //!< Half, availability depends on: `GRAPHICS_CAPS_VERTEX_ATTRIB_HALF`.
			Float,  //!< Float
	...
	///
	/// @attention C99's equivalent binding is `graphics_texture_format_t`.
	///
	...
	/// Uniform type enum.
	///
	/// @attention C99's equivalent binding is `graphics_uniform_type_t`.
	///
	struct UniformType
	{
		/// Uniform types:
	...
	///
	/// @attention C99's equivalent binding is `graphics_backbuffer_ratio_t`.
	///
	...
	///
	/// @attention C99's equivalent binding is `graphics_occlusion_query_result_t`.
	///
	...
	/// Primitive topology.
	///
	/// @attention C99's equivalent binding is `graphics_topology_t`.
	///
	struct Topology
	{
		/// Primitive topology:
	...
	///
	/// @attention C99's equivalent binding is `graphics_topology_convert_t`.
	///
	...
	/// Topology sort order.
	///
	/// @attention C99's equivalent binding is `graphics_topology_sort_t`.
	///
	struct TopologySort
	{
		/// Topology sort order:
	...
	///
	/// @attention C99's equivalent binding is `graphics_view_mode_t`.
	///
	struct ViewMode
	{
		/// View modes:
	...
	GRAPHICS_HANDLE(DynamicIndexBufferHandle)
	GRAPHICS_HANDLE(DynamicVertexBufferHandle)
	GRAPHICS_HANDLE(FrameBufferHandle)
	GRAPHICS_HANDLE(IndexBufferHandle)
	GRAPHICS_HANDLE(IndirectBufferHandle)
	GRAPHICS_HANDLE(OcclusionQueryHandle)
	GRAPHICS_HANDLE(ProgramHandle)
	GRAPHICS_HANDLE(ShaderHandle)
	GRAPHICS_HANDLE(TextureHandle)
	GRAPHICS_HANDLE(UniformHandle)
	GRAPHICS_HANDLE(VertexBufferHandle)
	GRAPHICS_HANDLE(VertexLayoutHandle)
	...
	struct CallbackI
	{
		...
		///
		/// @attention C99's equivalent binding is `graphics_callback_vtbl.screen_shot`.
		///
		...
		/// @param[in] _yflip If true, image origin is bottom left.
		///
		/// @attention C99's equivalent binding is `graphics_callback_vtbl.capture_begin`.
		///
		...
		///
		/// @attention C99's equivalent binding is `graphics_callback_vtbl.capture_end`.
		///
		...
		/// @param[in] _size Image size.
		///
		/// @attention C99's equivalent binding is `graphics_callback_vtbl.capture_frame`.
		///
	...
	/// Platform data.
	///
	/// @attention C99's equivalent binding is `graphics_platform_data_t`.
	///
	struct PlatformData
	{
		PlatformData();
		...
		void* nwh;          //!< Native window handle. If `NULL`, graphics will create a headless
		                    ///  context/device, provided the rendering API supports it.
		void* context;      //!< GL context, D3D device, or Vulkan device. If `NULL`, graphics
		                    ///  will create context/device.
		void* backBuffer;   //!< GL back-buffer, or D3D render target view. If `NULL` graphics will
		                    ///  create back-buffer color surface.
		void* backBufferDS; //!< Backbuffer depth/stencil. If `NULL`, graphics will create a back-buffer
		                    ///  depth/stencil surface.
	};

	/// Backbuffer resolution and reset parameters.
	///
	/// @attention C99's equivalent binding is `graphics_resolution_t`.
	///
	struct Resolution
	{
		Resolution();
	...
	/// Initialization parameters used by `graphics::init`.
	///
	/// @attention C99's equivalent binding is `graphics_init_t`.
	///
	struct Init
	{
		Init();
		...
		///
		/// @attention C99's equivalent binding is `graphics_init_limits_t`.
		///
		struct Limits
		{
			Limits();
		...
		/// See: `graphics::CallbackI`
		CallbackI* callback;

		/// Custom allocator. When a custom allocator is not
		/// specified, graphics uses the CRT allocator. Bgfx assumes
		/// custom allocator is thread safe.
		base::AllocatorI* allocator;
	};
	...
	///
	/// @attention C99's equivalent binding is `graphics_release_fn_t`.
	///
	typedef void (*ReleaseFn)(void* _ptr, void* _userData);

	/// Memory must be obtained by calling `graphics::alloc`, `graphics::copy`, or `graphics::makeRef`.
	///
	/// @attention It is illegal to create this structure on stack and pass it to any graphics API.
	///
	/// @attention C99's equivalent binding is `graphics_memory_t`.
	///
	struct Memory
	{
		Memory() = delete;
	...
	///
	/// @attention C99's equivalent binding is `graphics_caps_t`.
	///
	struct Caps
	{
		/// Renderer backend type. See: `graphics::RendererType`
		RendererType::Enum rendererType;

		/// Supported functionality.
		///
		/// @attention See `GRAPHICS_CAPS_*` flags at https://bkaradzic.github.io/graphics/graphics.html#available-caps
		///
		uint64_t supported;
		...
		/// GPU info.
		///
		/// @attention C99's equivalent binding is `graphics_caps_gpu_t`.
		///
		struct GPU
		{
			uint16_t vendorId; //!< Vendor PCI id. See `GRAPHICS_PCI_ID_*`.
		...
		///
		/// @attention C99's equivalent binding is `graphics_caps_limits_t`.
		///
		struct Limits
		{
		...
		///   - `GRAPHICS_CAPS_FORMAT_TEXTURE_NONE` - Texture format is not supported.
		///   - `GRAPHICS_CAPS_FORMAT_TEXTURE_2D` - Texture format is supported.
		///   - `GRAPHICS_CAPS_FORMAT_TEXTURE_2D_SRGB` - Texture as sRGB format is supported.
		///   - `GRAPHICS_CAPS_FORMAT_TEXTURE_2D_EMULATED` - Texture format is emulated.
		///   - `GRAPHICS_CAPS_FORMAT_TEXTURE_3D` - Texture format is supported.
		///   - `GRAPHICS_CAPS_FORMAT_TEXTURE_3D_SRGB` - Texture as sRGB format is supported.
		///   - `GRAPHICS_CAPS_FORMAT_TEXTURE_3D_EMULATED` - Texture format is emulated.
		///   - `GRAPHICS_CAPS_FORMAT_TEXTURE_CUBE` - Texture format is supported.
		///   - `GRAPHICS_CAPS_FORMAT_TEXTURE_CUBE_SRGB` - Texture as sRGB format is supported.
		///   - `GRAPHICS_CAPS_FORMAT_TEXTURE_CUBE_EMULATED` - Texture format is emulated.
		///   - `GRAPHICS_CAPS_FORMAT_TEXTURE_VERTEX` - Texture format can be used from vertex shader.
		///   - `GRAPHICS_CAPS_FORMAT_TEXTURE_IMAGE_READ` - Texture format can be used as image
		///     and read from.
		///   - `GRAPHICS_CAPS_FORMAT_TEXTURE_IMAGE_WRITE` - Texture format can be used as image
		///     and written to.
		///   - `GRAPHICS_CAPS_FORMAT_TEXTURE_FRAMEBUFFER` - Texture format can be used as frame
		///     buffer.
		///   - `GRAPHICS_CAPS_FORMAT_TEXTURE_FRAMEBUFFER_MSAA` - Texture format can be used as MSAA
		///     frame buffer.
		///   - `GRAPHICS_CAPS_FORMAT_TEXTURE_MSAA` - Texture can be sampled as MSAA.
		///   - `GRAPHICS_CAPS_FORMAT_TEXTURE_MIP_AUTOGEN` - Texture format supports auto-generated
		///     mips.
	...
	///
	/// @attention C99's equivalent binding is `graphics_transient_index_buffer_t`.
	///
	...
	///
	/// @attention C99's equivalent binding is `graphics_transient_vertex_buffer_t`.
	///
	...
	///
	/// @attention C99's equivalent binding is `graphics_instance_data_buffer_t`.
	///
	...
	/// Texture info.
	///
	/// @attention C99's equivalent binding is `graphics_texture_info_t`.
	///
	struct TextureInfo
	{
	...
	/// Uniform info.
	///
	/// @attention C99's equivalent binding is `graphics_uniform_info_t`.
	///
	struct UniformInfo
	{
	...
	///
	/// @attention C99's equivalent binding is `graphics_attachment_t`.
	///
	struct Attachment
	{
		/// Init attachment.
		...
		/// @param[in] _resolve Resolve flags. See: `GRAPHICS_RESOLVE_*`
		///
		void init(
			...
			, uint16_t _mip = 0
			, uint8_t _resolve = GRAPHICS_RESOLVE_AUTO_GEN_MIPS
			);
		...
		uint8_t  resolve;     //!< Resolve flags. See: `GRAPHICS_RESOLVE_*`
	};

	/// Transform data.
	///
	/// @attention C99's equivalent binding is `graphics_transform_t`.
	///
	struct Transform
	{
	...
	/// View stats.
	///
	/// @attention C99's equivalent binding is `graphics_view_stats_t`.
	///
	struct ViewStats
	{
	...
	/// Encoder stats.
	///
	/// @attention C99's equivalent binding is `graphics_encoder_stats_t`.
	///
	struct EncoderStats
	{
	...
	///
	/// @attention C99's equivalent binding is `graphics_stats_t`.
	///
	/// @remarks All time values are high-resolution timestamps, while
	///   time frequencies define timestamps-per-second for that hardware.
	struct Stats
	{
		int64_t cpuTimeFrame;               //!< CPU time between two `graphics::frame` calls.
	...
	/// per thread should be used. Use `graphics::begin()` to obtain an encoder for a thread.
	///
	/// @attention C99's equivalent binding is `graphics_encoder`.
	///
	struct Encoder
	{
		...
		///
		/// @attention C99's equivalent binding is `graphics_encoder_set_marker`.
		///
		...
		///   triangles. See: `GRAPHICS_STATE_DEFAULT`.
		///   - `GRAPHICS_STATE_DEPTH_TEST_*` - Depth test function.
		///   - `GRAPHICS_STATE_BLEND_*` - See remark 1 about GRAPHICS_STATE_BLEND_FUNC.
		///   - `GRAPHICS_STATE_BLEND_EQUATION_*` - See remark 2.
		///   - `GRAPHICS_STATE_CULL_*` - Backface culling mode.
		///   - `GRAPHICS_STATE_WRITE_*` - Enable R, G, B, A or Z write.
		///   - `GRAPHICS_STATE_MSAA` - Enable hardware multisample antialiasing.
		///   - `GRAPHICS_STATE_PT_[TRISTRIP/LINES/POINTS]` - Primitive type.
		///
		/// @param[in] _rgba Sets blend factor used by `GRAPHICS_STATE_BLEND_FACTOR` and
		///   `GRAPHICS_STATE_BLEND_INV_FACTOR` blend modes.
		///
		/// @remarks
		///   1. To set up more complex states use:
		///      `GRAPHICS_STATE_ALPHA_REF(_ref)`,
		///      `GRAPHICS_STATE_POINT_SIZE(_size)`,
		///      `GRAPHICS_STATE_BLEND_FUNC(_src, _dst)`,
		///      `GRAPHICS_STATE_BLEND_FUNC_SEPARATE(_srcRGB, _dstRGB, _srcA, _dstA)`,
		///      `GRAPHICS_STATE_BLEND_EQUATION(_equation)`,
		///      `GRAPHICS_STATE_BLEND_EQUATION_SEPARATE(_equationRGB, _equationA)`
		///   2. `GRAPHICS_STATE_BLEND_EQUATION_ADD` is set when no other blend
		///      equation is specified.
		///
		/// @attention C99's equivalent binding is `graphics_encoder_set_state`.
		///
		void setState(
			  uint64_t _state
			, uint32_t _rgba = 0
		...
		///
		/// @attention C99's equivalent binding is `graphics_encoder_set_condition`.
		///
		void setCondition(
		...
		/// @param[in] _bstencil Back stencil state. If back is set to `GRAPHICS_STENCIL_NONE`
		///   _fstencil is applied to both front and back facing primitives.
		///
		/// @attention C99's equivalent binding is `graphics_encoder_set_stencil`.
		///
		void setStencil(
			  uint32_t _fstencil
			, uint32_t _bstencil = GRAPHICS_STENCIL_NONE
			);

		/// Set scissor for draw primitive. To scissor for all primitives in
		/// view see `graphics::setViewScissor`.
		///
		...
		///
		/// @attention C99's equivalent binding is `graphics_encoder_set_scissor`.
		///
		uint16_t setScissor(
			  uint16_t _x
			, uint16_t _y
		...
		///
		/// @attention C99's equivalent binding is `graphics_encoder_set_scissor_cached`.
		///
		...
		///
		/// @attention C99's equivalent binding is `graphics_encoder_set_transform`.
		///
		...
		///
		/// @attention Pointer returned can be modified until `graphics::frame` is called.
		/// @attention C99's equivalent binding is `graphics_encoder_alloc_transform`.
		///
		...
		///
		/// @attention C99's equivalent binding is `graphics_encoder_set_transform_cached`.
		///
		void setTransform(
			  uint32_t _cache
			, uint16_t _num = 1
		...
		///
		/// @attention C99's equivalent binding is `graphics_encoder_set_uniform`.
		///
		void setUniform(
		...
		///
		/// @attention C99's equivalent binding is `graphics_encoder_set_index_buffer`.
		///
		...
		///
		/// @attention C99's equivalent binding is `graphics_encoder_set_dynamic_index_buffer`.
		///
		...
		///
		/// @attention C99's equivalent binding is `graphics_encoder_set_transient_index_buffer`.
		///
		...
		///
		/// @attention C99's equivalent binding is `graphics_encoder_set_vertex_buffer`.
		///
		...
		void setVertexBuffer(
			...
			, VertexLayoutHandle _layoutHandle = GRAPHICS_INVALID_HANDLE
			);
		...
		///
		/// @attention C99's equivalent binding is `graphics_encoder_set_dynamic_vertex_buffer`.
		///
		void setVertexBuffer(
			  uint8_t _stream
		...
		///
		/// @attention C99's equivalent binding is `graphics_encoder_set_transient_vertex_buffer`.
		///
		void setVertexBuffer(
			  uint8_t _stream
		...
		///
		/// @attention Availability depends on: `GRAPHICS_CAPS_VERTEX_ID`.
		/// @attention C99's equivalent binding is `graphics_encoder_set_vertex_count`.
		///
		...
		///
		/// @attention C99's equivalent binding is `graphics_encoder_set_instance_data_buffer`.
		///
		...
		/// @param[in] _num Number of data instances.
		///
		/// @attention C99's equivalent binding is `graphics_encoder_set_instance_data_from_vertex_buffer`.
		///
		void setInstanceDataBuffer(
			  VertexBufferHandle _handle
			, uint32_t _start
		...
		/// @param[in] _num Number of data instances.
		///
		/// @attention C99's equivalent binding is `graphics_encoder_set_instance_data_from_dynamic_vertex_buffer`.
		///
		void setInstanceDataBuffer(
			  DynamicVertexBufferHandle _handle
			, uint32_t _start
		...
		///
		/// @attention Availability depends on: `GRAPHICS_CAPS_VERTEX_ID`.
		/// @attention C99's equivalent binding is `graphics_encoder_set_instance_count`.
		///
		...
		///     sampling.
		///
		/// @attention C99's equivalent binding is `graphics_encoder_set_texture`.
		///
		void setTexture(
			  uint8_t _stage
		...
		///
		/// @attention C99's equivalent binding is `graphics_encoder_touch`.
		///
		...
		/// @param[in] _flags Discard or preserve states. See `GRAPHICS_DISCARD_*`.
		///
		/// @attention C99's equivalent binding is `graphics_encoder_submit`.
		///
		void submit(
			  ViewId _id
			, ProgramHandle _program
			, uint32_t _depth = 0
			, uint8_t _flags  = GRAPHICS_DISCARD_ALL
			);
		...
		/// @param[in] _flags Discard or preserve states. See `GRAPHICS_DISCARD_*`.
		///
		/// @attention C99's equivalent binding is `graphics_encoder_submit_occlusion_query`.
		///
		void submit(
			  ViewId _id
			, ProgramHandle _program
		...
		/// @param[in] _flags Discard or preserve states. See `GRAPHICS_DISCARD_*`.
		///
		/// @attention Availability depends on: `GRAPHICS_CAPS_DRAW_INDIRECT`.
		/// @attention C99's equivalent binding is `graphics_encoder_submit_indirect`.
		///
		void submit(
			  ViewId _id
			, ProgramHandle _program
		...
		///   with `GRAPHICS_BUFFER_INDEX32` and `GRAPHICS_BUFFER_DRAW_INDIRECT`.
		/// @param[in] _numIndex Element in number buffer.
		/// @param[in] _numMax Max number of draws.
		/// @param[in] _depth Depth for sorting.
		/// @param[in] _flags Discard or preserve states. See `GRAPHICS_DISCARD_*`.
		///
		/// @attention Availability depends on: `GRAPHICS_CAPS_DRAW_INDIRECT_COUNT`.
		/// @attention C99's equivalent binding is `graphics_encoder_submit_indirect_count`.
		///
		void submit(
			  ViewId _id
			, ProgramHandle _program
		...
		///
		/// @attention C99's equivalent binding is `graphics_encoder_set_compute_index_buffer`.
		///
		void setBuffer(
			  uint8_t _stage
			, IndexBufferHandle _handle
		...
		/// @param[in] _access Buffer access. See `Access::Enum`.
		///
		/// @attention C99's equivalent binding is `graphics_encoder_set_compute_vertex_buffer`.
		///
		void setBuffer(
			  uint8_t _stage
			, VertexBufferHandle _handle
		...
		/// @param[in] _access Buffer access. See `Access::Enum`.
		///
		/// @attention C99's equivalent binding is `graphics_encoder_set_compute_dynamic_index_buffer`.
		///
		void setBuffer(
			  uint8_t _stage
			, DynamicIndexBufferHandle _handle
		...
		/// @param[in] _access Buffer access. See `Access::Enum`.
		///
		/// @attention C99's equivalent binding is `graphics_encoder_set_compute_dynamic_vertex_buffer`.
		///
		void setBuffer(
			  uint8_t _stage
			, DynamicVertexBufferHandle _handle
		...
		/// @param[in] _access Buffer access. See `Access::Enum`.
		///
		/// @attention C99's equivalent binding is `graphics_encoder_set_compute_indirect_buffer`.
		///
		void setBuffer(
			  uint8_t _stage
		...
		/// @param[in] _format Texture format. See: `TextureFormat::Enum`.
		///
		/// @attention C99's equivalent binding is `graphics_encoder_set_image`.
		///
		void setImage(
			  uint8_t _stage
			, TextureHandle _handle
		...
		/// @param[in] _flags Discard or preserve states. See `GRAPHICS_DISCARD_*`.
		///
		/// @attention C99's equivalent binding is `graphics_encoder_dispatch`.
		///
		void dispatch(
			  ViewId _id
		...
		/// @param[in] _flags Discard or preserve states. See `GRAPHICS_DISCARD_*`.
		///
		/// @attention C99's equivalent binding is `graphics_encoder_dispatch_indirect`.
		///
		void dispatch(
			  ViewId _id
			, ProgramHandle _handle
		...
		///
		/// @attention C99's equivalent binding is `graphics_encoder_discard`.
		///
		void discard(uint8_t _flags = GRAPHICS_DISCARD_ALL);
		...
		///
		/// @attention Destination texture must be created with `GRAPHICS_TEXTURE_BLIT_DST` flag.
		/// @attention Availability depends on: `GRAPHICS_CAPS_TEXTURE_BLIT`.
		/// @attention C99's equivalent binding is `graphics_encoder_blit`.
		///
		void blit(
			  ViewId _id
			, TextureHandle _dst
	...
	/// Vertex layout.
	///
	/// @attention C99's equivalent binding is `graphics_vertex_layout_t`.
	///
	struct VertexLayout
	{
		VertexLayout();
		...
		/// Start VertexLayout.
		///
		/// @param[in] _renderer Renderer backend type. See: `graphics::RendererType`
		/// @returns Returns itself.
		///
		/// @attention C99's equivalent binding is `graphics_vertex_layout_begin`.
		///
		VertexLayout& begin(RendererType::Enum _renderer = RendererType::Noop);

		/// End VertexLayout.
		///
		/// @attention C99's equivalent binding is `graphics_vertex_layout_end`.
		///
		void end();

		/// Add attribute to VertexLayout.
		///
		/// @param[in] _attrib Attribute semantics. See: `graphics::Attrib`
		...
		///
		/// @attention C99's equivalent binding is `graphics_vertex_layout_add`.
		///
		VertexLayout& add(
			  Attrib::Enum _attrib
			, uint8_t _num
		...
		/// @returns Returns itself.
		///
		/// @attention C99's equivalent binding is `graphics_vertex_layout_skip`.
		///
		VertexLayout& skip(uint8_t _num);

		/// Decode attribute.
		///
		/// @attention C99's equivalent binding is `graphics_vertex_layout_decode`.
		///
		void decode(
			  Attrib::Enum _attrib
			, uint8_t& _num
		...
		/// @returns True if VertexLayout contains attribute.
		///
		/// @attention C99's equivalent binding is `graphics_vertex_layout_has`.
		///
	...
	///
	/// @attention C99's equivalent binding is `graphics_vertex_pack`.
	///
	void vertexPack(
	...
	///
	/// @attention C99's equivalent binding is `graphics_vertex_unpack`.
	///
	void vertexUnpack(
		  float _output[4]
		, Attrib::Enum _attr
	...
	///
	/// @attention C99's equivalent binding is `graphics_vertex_convert`.
	///
	void vertexConvert(
	...
	///
	/// @attention C99's equivalent binding is `graphics_weld_vertices`.
	///
	...
	///
	/// @attention C99's equivalent binding is `graphics_topology_convert`.
	///
	...
	/// @param[in] _index32 Set to `true` if input indices are 32-bit.
	///
	/// @attention C99's equivalent binding is `graphics_topology_sort_tri_list`.
	///
	...
	///
	/// @attention C99's equivalent binding is `graphics_get_supported_renderers`.
	///
	...
	///
	/// @attention C99's equivalent binding is `graphics_get_renderer_name`.
	///
	const char* getRendererName(RendererType::Enum _type);

	/// Initialize the graphics library.
	///
	/// @param[in] _init Initialization parameters. See: `graphics::Init` for more info.
	///
	/// @returns `true` if initialization was successful.
	///
	/// @attention C99's equivalent binding is `graphics_init`.
	///
	bool init(const Init& _init = {});

	/// Shutdown graphics library.
	///
	/// @attention C99's equivalent binding is `graphics_shutdown`.
	///
	void shutdown();
	...
	/// @param[in] _flags See: `GRAPHICS_RESET_*` for more info.
	...
	///
	/// @attention C99's equivalent binding is `graphics_reset`.
	///
	void reset(
		  uint32_t _width
		, uint32_t _height
		, uint32_t _flags = GRAPHICS_RESET_NONE
		, TextureFormat::Enum _format = TextureFormat::Count
		);
	...
	///   library via `graphics::makeRef` calls.
	///
	/// @attention C99's equivalent binding is `graphics_frame`.
	///
	...
	///
	/// @attention C99's equivalent binding is `graphics_get_renderer_type`.
	///
	RendererType::Enum getRendererType();

	/// Returns renderer capabilities.
	///
	/// @returns Pointer to static `graphics::Caps` structure.
	///
	/// @remarks
	///   Library must be initialized.
	///
	/// @attention C99's equivalent binding is `graphics_get_caps`.
	///
	const Caps* getCaps();

	/// Returns performance counters.
	///
	/// @attention Pointer returned is valid until `graphics::frame` is called.
	/// @attention C99's equivalent binding is `graphics_get_stats`.
	///
	const Stats* getStats();

	/// Allocate buffer to pass to graphics calls. Data will be freed inside graphics.
	///
	/// @param[in] _size Size to allocate.
	///
	/// @attention C99's equivalent binding is `graphics_alloc`.
	///
	const Memory* alloc(uint32_t _size);

	/// Allocate buffer and copy data into it. Data will be freed inside graphics.
	///
	/// @param[in] _data Pointer to data to be copied.
	/// @param[in] _size Size of data to be copied.
	///
	/// @attention C99's equivalent binding is `graphics_copy`.
	///
	const Memory* copy(
		  const void* _data
		, uint32_t _size
	...
	/// Make reference to data to pass to graphics. Unlike `graphics::alloc`, this call
	/// doesn't allocate memory for data. It just copies the _data pointer. You
	/// can pass `ReleaseFn` function pointer to release this memory after it's
	/// consumed, otherwise you must make sure _data is available for at least 2
	/// `graphics::frame` calls. `ReleaseFn` function must be able to be called
	/// from any thread.
	///
	...
	///
	/// @attention Data passed must be available for at least 2 `graphics::frame` calls.
	/// @attention C99's equivalent bindings are `graphics_make_ref`, `graphics_make_ref_release`.
	///
	const Memory* makeRef(
		  const void* _data
		, uint32_t _size
		, base::AllocatorI* _customAlloc = NULL
	...
	///
	/// @attention C99's equivalent binding is `graphics_set_debug`.
	///
	...
	///
	/// @attention C99's equivalent binding is `graphics_dbg_text_clear`.
	///
	void dbgTextClear(
		  uint8_t _attr = 0
		, bool _small = false
	...
	/// @param[in] _format `printf` style format.
	///
	/// @attention C99's equivalent binding is `graphics_dbg_text_printf`.
	///
	void dbgTextPrintf(
		  uint16_t _x
		, uint16_t _y
	...
	///
	/// @attention C99's equivalent binding is `graphics_dbg_text_vprintf`.
	///
	...
	///
	/// @attention C99's equivalent binding is `graphics_dbg_text_image`.
	///
	void dbgTextImage(
		  uint16_t _x
		, uint16_t _y
	...
	///   - `GRAPHICS_BUFFER_NONE` - No flags.
	///   - `GRAPHICS_BUFFER_COMPUTE_READ` - Buffer will be read from by compute shader.
	///   - `GRAPHICS_BUFFER_COMPUTE_WRITE` - Buffer will be written into by compute shader. When buffer
	///       is created with `GRAPHICS_BUFFER_COMPUTE_WRITE` flag it cannot be updated from CPU.
	///   - `GRAPHICS_BUFFER_COMPUTE_READ_WRITE` - Buffer will be used for read/write by compute shader.
	///   - `GRAPHICS_BUFFER_ALLOW_RESIZE` - Buffer will resize on buffer update if a different amount of
	///       data is passed. If this flag is not specified, and more data is passed on update, the buffer
	///       will be trimmed to fit the existing buffer size. This flag has effect only on dynamic
	///       buffers.
	///   - `GRAPHICS_BUFFER_INDEX32` - Buffer is using 32-bit indices. This flag has effect only on
	///       index buffers.
	///
	/// @attention C99's equivalent binding is `graphics_create_index_buffer`.
	///
	IndexBufferHandle createIndexBuffer(
		  const Memory* _mem
		, uint16_t _flags = GRAPHICS_BUFFER_NONE
		);
	...
	///
	/// @attention C99's equivalent binding is `graphics_set_index_buffer_name`.
	///
	void setName(
		  IndexBufferHandle _handle
		, const char* _name
	...
	/// @param[in] _handle Static index buffer handle.
	///
	/// @attention C99's equivalent binding is `graphics_destroy_index_buffer`.
	///
	void destroy(IndexBufferHandle _handle);

	/// Create vertex layout.
	///
	/// @attention C99's equivalent binding is `graphics_create_vertex_layout`.
	///
	VertexLayoutHandle createVertexLayout(const VertexLayout& _layout);

	/// Destroy vertex layout.
	///
	/// @attention C99's equivalent binding is `graphics_destroy_vertex_layout`.
	///
	...
	///
	/// @attention C99's equivalent binding is `graphics_create_vertex_buffer`.
	///
	...
	///   that _name is zero terminated string.
	///
	/// @attention C99's equivalent binding is `graphics_set_vertex_buffer_name`.
	///
	void setName(
		  VertexBufferHandle _handle
		, const char* _name
	...
	/// @param[in] _handle Static vertex buffer handle.
	///
	/// @attention C99's equivalent binding is `graphics_destroy_vertex_buffer`.
	///
	...
	///
	/// @attention C99's equivalent binding is `graphics_create_dynamic_index_buffer`.
	///
	...
	/// @returns Dynamic index buffer handle.
	///
	/// @attention C99's equivalent binding is `graphics_create_dynamic_index_buffer_mem`.
	///
	DynamicIndexBufferHandle createDynamicIndexBuffer(
		  const Memory* _mem
		, uint16_t _flags = GRAPHICS_BUFFER_NONE
	...
	/// @param[in] _mem Index buffer data.
	///
	/// @attention C99's equivalent binding is `graphics_update_dynamic_index_buffer`.
	///
	void update(
		  DynamicIndexBufferHandle _handle
	...
	/// @param[in] _handle Dynamic index buffer handle.
	///
	/// @attention C99's equivalent binding is `graphics_destroy_dynamic_index_buffer`.
	///
	...
	///
	/// @attention C99's equivalent binding is `graphics_create_dynamic_vertex_buffer`.
	///
	...
	/// @returns Dynamic vertex buffer handle.
	///
	/// @attention C99's equivalent binding is `graphics_create_dynamic_vertex_buffer_mem`.
	///
	DynamicVertexBufferHandle createDynamicVertexBuffer(
		  const Memory* _mem
		, const VertexLayout& _layout
	...
	/// @param[in] _mem Vertex buffer data.
	///
	/// @attention C99's equivalent binding is `graphics_update_dynamic_vertex_buffer`.
	///
	void update(
		  DynamicVertexBufferHandle _handle
		, uint32_t _startVertex
	...
	/// @param[in] _handle Dynamic vertex buffer handle.
	///
	/// @attention C99's equivalent binding is `graphics_destroy_dynamic_vertex_buffer`.
	///
	...
	///
	/// @attention C99's equivalent binding is `graphics_get_avail_transient_index_buffer`.
	///
	...
	/// @param[in] _layout Vertex layout.
	///
	/// @attention C99's equivalent binding is `graphics_get_avail_transient_vertex_buffer`.
	///
	...
	///
	/// @attention C99's equivalent binding is `graphics_get_avail_instance_data_buffer`.
	///
	...
	/// @param[in] _index32 Set to `true` if input indices will be 32-bit.
	///
	/// @attention C99's equivalent binding is `graphics_alloc_transient_index_buffer`.
	///
	...
	/// @param[in] _layout Vertex layout.
	///
	/// @attention C99's equivalent binding is `graphics_alloc_transient_vertex_buffer`.
	///
	...
	/// @param[in] _index32 Set to `true` if input indices will be 32-bit.
	///
	/// @attention C99's equivalent binding is `graphics_alloc_transient_buffers`.
	///
	...
	///
	/// @attention C99's equivalent binding is `graphics_alloc_instance_data_buffer`.
	///
	...
	///
	/// @attention C99's equivalent binding is `graphics_create_indirect_buffer`.
	///
	...
	/// @param[in] _handle Indirect buffer handle.
	///
	/// @attention C99's equivalent binding is `graphics_destroy_indirect_buffer`.
	///
	...
	///
	/// @attention C99's equivalent binding is `graphics_create_shader`.
	///
	...
	///
	/// @attention C99's equivalent binding is `graphics_get_shader_uniforms`.
	///
	...
	///   that _name is zero terminated string.
	///
	/// @attention C99's equivalent binding is `graphics_set_shader_name`.
	///
	void setName(
		  ShaderHandle _handle
		, const char* _name
	...
	/// @param[in] _handle Shader handle.
	///
	/// @attention C99's equivalent binding is `graphics_destroy_shader`.
	///
	...
	///
	/// @attention C99's equivalent binding is `graphics_create_program`.
	///
	...
	///
	/// @attention C99's equivalent binding is `graphics_create_compute_program`.
	///
	ProgramHandle createProgram(
		  ShaderHandle _csh
	...
	///
	/// @attention C99's equivalent binding is `graphics_destroy_program`.
	///
	...
	/// @param[in] _format Texture format. See: `TextureFormat::Enum`.
	/// @param[in] _flags Texture flags. See `GRAPHICS_TEXTURE_*`.
	/// @returns True if a texture with the same parameters can be created.
	///
	/// @attention C99's equivalent binding is `graphics_is_texture_valid`.
	///
	bool isTextureValid(
		  uint16_t _depth
		, bool _cubeMap
	...
	/// @param[in] _attachment Attachment texture info. See: `graphics::Attachment`.
	///
	...
	/// @param[in] _format Texture format. See: `TextureFormat::Enum`.
	///
	/// @attention C99's equivalent binding is `graphics_calc_texture_size`.
	///
	...
	/// @param[in] _flags Texture creation (see `GRAPHICS_TEXTURE_*`.), and sampler (see `GRAPHICS_SAMPLER_*`)
	...
	///
	/// @attention C99's equivalent binding is `graphics_create_texture`.
	///
	...
	///   `GRAPHICS_CAPS_TEXTURE_2D_ARRAY` flag is not set.
	/// @param[in] _format Texture format. See: `TextureFormat::Enum`.
	/// @param[in] _flags Texture creation (see `GRAPHICS_TEXTURE_*`.), and sampler (see `GRAPHICS_SAMPLER_*`)
	///   flags. Default texture sampling mode is linear, and wrap mode is repeat.
	///   - `GRAPHICS_SAMPLER_[U/V/W]_[MIRROR/CLAMP]` - Mirror or clamp to edge wrap
	...
	///
	/// @attention C99's equivalent binding is `graphics_create_texture_2d`.
	///
	...
	///     sampling.
	///
	/// @attention C99's equivalent binding is `graphics_create_texture_2d_scaled`.
	///
	TextureHandle createTexture2D(
	...
	///
	/// @attention C99's equivalent binding is `graphics_create_texture_3d`.
	///
	...
	/// @param[in] _hasMips Indicates that texture contains full mip-map chain.
	/// @param[in] _numLayers Number of layers in texture array. Must be 1 if caps
	///   `GRAPHICS_CAPS_TEXTURE_CUBE_ARRAY` flag is not set.
	/// @param[in] _format Texture format. See: `TextureFormat::Enum`.
	/// @param[in] _flags Texture creation (see `GRAPHICS_TEXTURE_*`.), and sampler (see `GRAPHICS_SAMPLER_*`)
	///   flags. Default texture sampling mode is linear, and wrap mode is repeat.
	///   - `GRAPHICS_SAMPLER_[U/V/W]_[MIRROR/CLAMP]` - Mirror or clamp to edge wrap
	...
	///   1, expected memory layout is texture and all mips together for each array element.
	///
	/// @attention C99's equivalent binding is `graphics_create_texture_cube`.
	///
	...
	///
	/// @attention It's valid to update only mutable texture. See `graphics::createTexture2D` for more info.
	///
	/// @attention C99's equivalent binding is `graphics_update_texture_2d`.
	///
	...
	/// @param[in] _mem Texture update data.
	///
	/// @attention It's valid to update only mutable texture. See `graphics::createTexture3D` for more info.
	///
	/// @attention C99's equivalent binding is `graphics_update_texture_3d`.
	///
	...
	/// @param[in] _handle Texture handle.
	/// @param[in] _layer Layers in texture array.
	/// @param[in] _side Cubemap side `GRAPHICS_CUBE_MAP_<POSITIVE or NEGATIVE>_<X, Y or Z>`,
	...
	///   UINT16_MAX, it will be calculated internally based on _width.
	///
	/// @attention It's valid to update only mutable texture. See `graphics::createTextureCube` for more info.
	///
	/// @attention C99's equivalent binding is `graphics_update_texture_cube`.
	///
	...
	/// @param[in] _mip Mip level.
	///
	/// @returns Frame number when the result will be available. See: `graphics::frame`.
	///
	/// @attention Texture must be created with `GRAPHICS_TEXTURE_READ_BACK` flag.
	/// @attention Availability depends on: `GRAPHICS_CAPS_TEXTURE_READ_BACK`.
	/// @attention C99's equivalent binding is `graphics_read_texture`.
	///
	...
	///   that _name is zero terminated string.
	///
	/// @attention C99's equivalent binding is `graphics_set_texture_name`.
	///
	void setName(
		  TextureHandle _handle
		, const char* _name
	...
	///
	/// @attention Availability depends on: `GRAPHICS_CAPS_TEXTURE_DIRECT_ACCESS`. This feature
	///   is available on GPUs that have unified memory architecture (UMA) support.
	///
	/// @attention C99's equivalent binding is `graphics_get_direct_access_ptr`.
	///
	...
	/// @param[in] _handle Texture handle.
	///
	/// @attention C99's equivalent binding is `graphics_destroy_texture`.
	///
	...
	///
	/// @attention C99's equivalent binding is `graphics_create_frame_buffer`.
	///
	FrameBufferHandle createFrameBuffer(
		  uint16_t _width
		, uint16_t _height
		, TextureFormat::Enum _format
		, uint64_t _textureFlags = GRAPHICS_SAMPLER_U_CLAMP|GRAPHICS_SAMPLER_V_CLAMP
		);
	...
	/// @returns Handle to frame buffer object.
	///
	/// @attention C99's equivalent binding is `graphics_create_frame_buffer_scaled`.
	///
	FrameBufferHandle createFrameBuffer(
		  BackbufferRatio::Enum _ratio
		, TextureFormat::Enum _format
	...
	/// @returns Handle to frame buffer object.
	///
	/// @attention C99's equivalent binding is `graphics_create_frame_buffer_from_handles`.
	///
	FrameBufferHandle createFrameBuffer(
		  uint8_t _num
	...
	/// @returns Handle to frame buffer object.
	///
	/// @attention C99's equivalent binding is `graphics_create_frame_buffer_from_attachment`.
	///
	FrameBufferHandle createFrameBuffer(
		  uint8_t _num
		, const Attachment* _attachment
	...
	///
	/// @attention C99's equivalent binding is `graphics_create_frame_buffer_from_nwh`.
	///
	FrameBufferHandle createFrameBuffer(
		  void* _nwh
		, uint16_t _width
	...
	///   that _name is zero terminated string.
	///
	/// @attention C99's equivalent binding is `graphics_set_frame_buffer_name`.
	///
	void setName(
	...
	///
	/// @attention C99's equivalent binding is `graphics_get_texture`.
	///
	...
	/// @param[in] _handle Frame buffer handle.
	///
	/// @attention C99's equivalent binding is `graphics_destroy_frame_buffer`.
	///
	...
	/// @param[in] _type Type of uniform (See: `graphics::UniformType`).
	/// @param[in] _num Number of elements in array.
	///
	/// @returns Handle to uniform object.
	///
	/// @remarks
	///   1. Uniform names are unique. It's valid to call `graphics::createUniform`
	///      multiple times with the same uniform name. The library will always
	///      return the same handle, but the handle reference count will be
	///      incremented. This means that the same number of `graphics::destroyUniform`
	///      must be called to properly destroy the uniform.
	///
	///   2. Predefined uniforms (declared in `graphics_shader.sh`):
	...
	///      - `u_model mat4[GRAPHICS_CONFIG_MAX_BONES]` - array of model matrices.
	///      - `u_modelView mat4` - concatenated model view matrix, only first
	///        model matrix from array is used.
	///      - `u_modelViewProj mat4` - concatenated model view projection matrix.
	///      - `u_alphaRef float` - alpha reference value for alpha test.
	///
	/// @attention C99's equivalent binding is `graphics_create_uniform`.
	///
	...
	///
	/// @attention C99's equivalent binding is `graphics_get_uniform_info`.
	///
	void getUniformInfo(
		  UniformHandle _handle
		, UniformInfo& _info
	...
	/// @param[in] _handle Handle to uniform object.
	///
	/// @attention C99's equivalent binding is `graphics_destroy_uniform`.
	///
	...
	///
	/// @attention C99's equivalent binding is `graphics_create_occlusion_query`.
	///
	...
	///
	/// @attention C99's equivalent binding is `graphics_get_result`.
	///
	...
	/// @param[in] _handle Handle to occlusion query object.
	///
	/// @attention C99's equivalent binding is `graphics_destroy_occlusion_query`.
	///
	...
	///
	/// @attention C99's equivalent binding is `graphics_set_palette_color`.
	///
	...
	///        +------ view id
	///
	/// @attention C99's equivalent binding is `graphics_set_view_name`.
	///
	void setViewName(
		  ViewId _id
		, const char* _name
	...
	///
	/// @attention C99's equivalent binding is `graphics_set_view_rect`.
	///
	void setViewRect(
		  ViewId _id
		, uint16_t _x
	...
	///   `BackbufferRatio::Enum`.
	///
	/// @attention C99's equivalent binding is `graphics_set_view_rect_ratio`.
	///
	void setViewRect(
		  ViewId _id
		, uint16_t _x
	...
	/// @param[in] _height Height of scissor region.
	///
	/// @attention C99's equivalent binding is `graphics_set_view_scissor`.
	///
	void setViewScissor(
		  ViewId _id
		, uint16_t _x = 0
	...
	///
	/// @param[in] _id View id.
	/// @param[in] _flags Clear flags. Use `GRAPHICS_CLEAR_NONE` to remove any clear
	///   operation. See: `GRAPHICS_CLEAR_*`.
	/// @param[in] _rgba Color clear value.
	/// @param[in] _depth Depth clear value.
	/// @param[in] _stencil Stencil clear value.
	///
	/// @attention C99's equivalent binding is `graphics_set_view_clear`.
	///
	void setViewClear(
		  ViewId _id
		, uint16_t _flags
	...
	/// frame buffer texture. `graphics::setPaletteColor` must be used to set up a
	/// clear color palette.
	///
	/// @param[in] _id View id.
	/// @param[in] _flags Clear flags. Use `GRAPHICS_CLEAR_NONE` to remove any clear
	...
	///
	/// @attention C99's equivalent binding is `graphics_set_view_clear_mrt`.
	///
	void setViewClear(
		  ViewId _id
		, uint16_t _flags
	...
	///
	/// @remarks
	///   View mode must be set prior calling `graphics::submit` for the view.
	///
	/// @attention C99's equivalent binding is `graphics_set_view_mode`.
	///
	void setViewMode(
		  ViewId _id
	...
	///
	/// @param[in] _id View id.
	/// @param[in] _handle Frame buffer handle. Passing `GRAPHICS_INVALID_HANDLE` as
	///   frame buffer handle will draw primitives from this view into
	///   default back buffer.
	///
	/// @remarks
	///   Not persistent after `graphics::reset` call.
	///
	/// @attention C99's equivalent binding is `graphics_set_view_frame_buffer`.
	///
	...
	///
	/// @attention C99's equivalent binding is `graphics_set_view_transform`.
	///
	...
	///   to default state.
	///
	/// @attention C99's equivalent binding is `graphics_set_view_order`.
	///
	void setViewOrder(
		  ViewId _id = 0
	...
	/// @param[in] _id View id.
	///
	/// @attention C99's equivalent binding is `graphics_reset_view`.
	///
	void resetView(ViewId _id);

	/// Sets debug marker.
	///
	/// @attention C99's equivalent binding is `graphics_set_marker`.
	///
	void setMarker(const char* _marker);
	...
	///   - `GRAPHICS_STATE_CULL_*` - Backface culling mode.
	///   - `GRAPHICS_STATE_WRITE_*` - Enable R, G, B, A or Z write.
	///   - `GRAPHICS_STATE_MSAA` - Enable MSAA.
	///   - `GRAPHICS_STATE_PT_[TRISTRIP/LINES/POINTS]` - Primitive type.
	///
	/// @param[in] _rgba Sets blend factor used by `GRAPHICS_STATE_BLEND_FACTOR` and
	///   `GRAPHICS_STATE_BLEND_INV_FACTOR` blend modes.
	...
	///      equation is specified.
	///
	/// @attention C99's equivalent binding is `graphics_set_state`.
	///
	void setState(
		  uint64_t _state
		, uint32_t _rgba = 0
	...
	/// @param[in] _visible Render if occlusion query is visible.
	///
	/// @attention C99's equivalent binding is `graphics_set_condition`.
	///
	void setCondition(
		  OcclusionQueryHandle _handle
		, bool _visible
	...
	///   _fstencil is applied to both front and back facing primitives.
	///
	/// @attention C99's equivalent binding is `graphics_set_stencil`.
	///
	void setStencil(
		  uint32_t _fstencil
		, uint32_t _bstencil = GRAPHICS_STENCIL_NONE
	...
	/// @returns Scissor cache index.
	///
	/// @attention C99's equivalent binding is `graphics_set_scissor`.
	///
	uint16_t setScissor(
		  uint16_t _x
		, uint16_t _y
	...
	///
	/// @attention C99's equivalent binding is `graphics_set_scissor_cached`.
	///
	void setScissor(uint16_t _cache = UINT16_MAX);
	...
	///   to be used for other draw primitive call.
	///
	/// @attention C99's equivalent binding is `graphics_set_transform`.
	///
	uint32_t setTransform(
		  const void* _mtx
		, uint16_t _num = 1
	...
	///
	/// @attention Pointer returned can be modified until `graphics::frame` is called.
	/// @attention C99's equivalent binding is `graphics_alloc_transform`.
	///
	uint32_t allocTransform(
		  Transform* _transform
		, uint16_t _num
	...
	/// @param[in] _num Number of matrices from cache.
	///
	/// @attention C99's equivalent binding is `graphics_set_transform_cached`.
	///
	void setTransform(
		  uint32_t _cache
		, uint16_t _num = 1
	...
	///   use the _num passed on uniform creation.
	///
	/// @attention C99's equivalent binding is `graphics_set_uniform`.
	///
	void setUniform(
		  UniformHandle _handle
		, const void* _value
	...
	/// @param[in] _handle Index buffer.
	///
	/// @attention C99's equivalent binding is `graphics_set_index_buffer`.
	///
	void setIndexBuffer(IndexBufferHandle _handle);
	...
	/// @param[in] _handle Dynamic index buffer.
	///
	/// @attention C99's equivalent binding is `graphics_set_dynamic_index_buffer`.
	///
	void setIndexBuffer(DynamicIndexBufferHandle _handle);
	...
	/// @param[in] _tib Transient index buffer.
	///
	/// @attention C99's equivalent binding is `graphics_set_transient_index_buffer`.
	///
	void setIndexBuffer(const TransientIndexBuffer* _tib);
	...
	/// @param[in] _handle Vertex buffer.
	///
	/// @attention C99's equivalent binding is `graphics_set_vertex_buffer`.
	///
	void setVertexBuffer(
		  uint8_t _stream
		, VertexBufferHandle _handle
	...
	/// @param[in] _handle Dynamic vertex buffer.
	///
	/// @attention C99's equivalent binding is `graphics_set_dynamic_vertex_buffer`.
	///
	void setVertexBuffer(
		  uint8_t _stream
		, DynamicVertexBufferHandle _handle
	...
	/// @param[in] _tvb Transient vertex buffer.
	///
	/// @attention C99's equivalent binding is `graphics_set_transient_vertex_buffer`.
	///
	void setVertexBuffer(
		  uint8_t _stream
		, const TransientVertexBuffer* _tvb
	...
	///
	/// @attention Availability depends on: `GRAPHICS_CAPS_VERTEX_ID`.
	/// @attention C99's equivalent binding is `graphics_set_vertex_count`.
	///
	void setVertexCount(uint32_t _numVertices);
	...
	/// @param[in] _idb Transient instance data buffer.
	///
	/// @attention C99's equivalent binding is `graphics_set_instance_data_buffer`.
	///
	void setInstanceDataBuffer(const InstanceDataBuffer* _idb);
	...
	/// @param[in] _num Number of data instances.
	///
	/// @attention C99's equivalent binding is `graphics_set_instance_data_from_vertex_buffer`.
	///
	void setInstanceDataBuffer(
		  VertexBufferHandle _handle
		, uint32_t _start
	...
	/// @param[in] _num Number of data instances.
	///
	/// @attention C99's equivalent binding is `graphics_set_instance_data_from_dynamic_vertex_buffer`.
	///
	void setInstanceDataBuffer(
		  DynamicVertexBufferHandle _handle
		, uint32_t _start
	...
	///
	/// @attention Availability depends on: `GRAPHICS_CAPS_VERTEX_ID`.
	/// @attention C99's equivalent binding is `graphics_set_instance_count`.
	///
	void setInstanceCount(uint32_t _numInstances);
	...
	///     sampling.
	///
	/// @attention C99's equivalent binding is `graphics_set_texture`.
	///
	void setTexture(
		  uint8_t _stage
		, UniformHandle _sampler
	...
	/// @param[in] _id View id.
	///
	/// @attention C99's equivalent binding is `graphics_touch`.
	///
	void touch(ViewId _id);
	...
	/// @param[in] _flags Discard or preserve states. See `GRAPHICS_DISCARD_*`.
	///
	/// @attention C99's equivalent binding is `graphics_submit`.
	///
	void submit(
		  ViewId _id
		, ProgramHandle _program
	...
	/// @param[in] _flags Discard or preserve states. See `GRAPHICS_DISCARD_*`.
	///
	/// @attention C99's equivalent binding is `graphics_submit_occlusion_query`.
	///
	void submit(
		  ViewId _id
		, ProgramHandle _program
	...
	///
	/// @attention Availability depends on: `GRAPHICS_CAPS_DRAW_INDIRECT`.
	/// @attention C99's equivalent binding is `graphics_submit_indirect`.
	///
	void submit(
		  ViewId _id
		, ProgramHandle _program
	...
	///
	/// @attention Availability depends on:`GRAPHICS_CAPS_DRAW_INDIRECT_COUNT`.
	/// @attention C99's equivalent binding is `graphics_submit_indirect_count`.
	///
	void submit(
		  ViewId _id
		, ProgramHandle _program
	...
	/// @param[in] _access Buffer access. See `Access::Enum`.
	///
	/// @attention C99's equivalent binding is `graphics_set_compute_index_buffer`.
	///
	void setBuffer(
		  uint8_t _stage
		, IndexBufferHandle _handle
	...
	/// @param[in] _access Buffer access. See `Access::Enum`.
	///
	/// @attention C99's equivalent binding is `graphics_set_compute_vertex_buffer`.
	///
	void setBuffer(
		  uint8_t _stage
		, VertexBufferHandle _handle
	...
	/// @param[in] _access Buffer access. See `Access::Enum`.
	///
	/// @attention C99's equivalent binding is `graphics_set_compute_dynamic_index_buffer`.
	///
	void setBuffer(
		  uint8_t _stage
		, DynamicIndexBufferHandle _handle
	...
	/// @param[in] _access Buffer access. See `Access::Enum`.
	///
	/// @attention C99's equivalent binding is `graphics_set_compute_dynamic_vertex_buffer`.
	///
	void setBuffer(
		  uint8_t _stage
		, DynamicVertexBufferHandle _handle
	...
	/// @param[in] _access Buffer access. See `Access::Enum`.
	///
	/// @attention C99's equivalent binding is `graphics_set_compute_indirect_buffer`.
	///
	void setBuffer(
		  uint8_t _stage
		, IndirectBufferHandle _handle
	...
	/// @param[in] _format Texture format. See: `TextureFormat::Enum`.
	///
	/// @attention C99's equivalent binding is `graphics_set_image`.
	///
	void setImage(
		  uint8_t _stage
		, TextureHandle _handle
	...
	/// @param[in] _flags Discard or preserve states. See `GRAPHICS_DISCARD_*`.
	///
	/// @attention C99's equivalent binding is `graphics_dispatch`.
	///
	void dispatch(
		  ViewId _id
		, ProgramHandle _handle
	...
	/// @param[in] _flags Discard or preserve states. See `GRAPHICS_DISCARD_*`.
	///
	/// @attention C99's equivalent binding is `graphics_dispatch_indirect`.
	///
	void dispatch(
		  ViewId _id
		, ProgramHandle _handle
	...
	/// @param[in] _flags Draw/compute states to discard.
	///
	/// @attention C99's equivalent binding is `graphics_discard`.
	///
	void discard(uint8_t _flags = GRAPHICS_DISCARD_ALL);
	...
	/// @attention Destination texture must be created with `GRAPHICS_TEXTURE_BLIT_DST` flag.
	/// @attention Availability depends on: `GRAPHICS_CAPS_TEXTURE_BLIT`.
	/// @attention C99's equivalent binding is `graphics_blit`.
	///
	void blit(
		  ViewId _id
		, TextureHandle _dst
	...
	///
	/// @param[in] _handle Frame buffer handle. If handle is `GRAPHICS_INVALID_HANDLE` request will be
	///   made for main window back buffer.
	/// @param[in] _filePath Will be passed to `graphics::CallbackI::screenShot` callback.
	///
	/// @remarks
	///   `graphics::CallbackI::screenShot` must be implemented.
	///
	/// @attention Frame buffer handle must be created with OS' target native window handle.
	/// @attention C99's equivalent binding is `graphics_request_screen_shot`.
	///
