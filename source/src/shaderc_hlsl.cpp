...
namespace shaderc {
	namespace hlsl
	{
		...
		const D3DCompiler* load(base::WriterI* _messageWriter)
		{
			base::Error messageErr;

			for (uint32_t ii = 0; ii < BASE_COUNTOF(s_d3dcompiler); ++ii)
			{
				const D3DCompiler* compiler = &s_d3dcompiler[ii];
				s_d3dcompilerdll = base::dlopen(compiler->fileName);
				if (NULL == s_d3dcompilerdll)
				{
					continue;
				}

				D3DCompile = (PFN_D3D_COMPILE)base::dlsym(s_d3dcompilerdll, "D3DCompile");
				D3DDisassemble = (PFN_D3D_DISASSEMBLE)base::dlsym(s_d3dcompilerdll, "D3DDisassemble");
				D3DReflect = (PFN_D3D_REFLECT)base::dlsym(s_d3dcompilerdll, "D3DReflect");
				D3DStripShader = (PFN_D3D_STRIP_SHADER)base::dlsym(s_d3dcompilerdll, "D3DStripShader");
				...
				if (NULL == D3DCompile
					...
					|| NULL == D3DStripShader)
				{
					base::dlclose(s_d3dcompilerdll);
					continue;
				}

				if (g_verbose)
				{
					char filePath[base::kMaxFilePath];
					GetModuleFileNameA((HMODULE)s_d3dcompilerdll, filePath, sizeof(filePath));
					BASE_TRACE("Loaded %s compiler (%s).", compiler->fileName, filePath);
				}

				return compiler;
			}

			base::write(_messageWriter, &messageErr, "Error: Unable to open D3DCompiler_*.dll shader compiler.\n");
			return NULL;
		}
		...
		{
			graphics::Attrib::Enum m_attr;
			const char* m_name;
			uint8_t m_index;
		};

		static const RemapInputSemantic s_remapInputSemantic[graphics::Attrib::Count + 1] =
		{
			{ graphics::Attrib::Position,  "POSITION",     0 },
			{ graphics::Attrib::Normal,    "NORMAL",       0 },
			{ graphics::Attrib::Tangent,   "TANGENT",      0 },
			{ graphics::Attrib::Bitangent, "BITANGENT",    0 },
			{ graphics::Attrib::Color0,    "COLOR",        0 },
			{ graphics::Attrib::Color1,    "COLOR",        1 },
			{ graphics::Attrib::Color2,    "COLOR",        2 },
			{ graphics::Attrib::Color3,    "COLOR",        3 },
			{ graphics::Attrib::Indices,   "BLENDINDICES", 0 },
			{ graphics::Attrib::Weight,    "BLENDWEIGHT",  0 },
			{ graphics::Attrib::TexCoord0, "TEXCOORD",     0 },
			{ graphics::Attrib::TexCoord1, "TEXCOORD",     1 },
			{ graphics::Attrib::TexCoord2, "TEXCOORD",     2 },
			{ graphics::Attrib::TexCoord3, "TEXCOORD",     3 },
			{ graphics::Attrib::TexCoord4, "TEXCOORD",     4 },
			{ graphics::Attrib::TexCoord5, "TEXCOORD",     5 },
			{ graphics::Attrib::TexCoord6, "TEXCOORD",     6 },
			{ graphics::Attrib::TexCoord7, "TEXCOORD",     7 },
			{ graphics::Attrib::Count,     "",             0 },
		};

		const RemapInputSemantic& findInputSemantic(const char* _name, uint8_t _index)
		{
			for (uint32_t ii = 0; ii < graphics::Attrib::Count; ++ii)
			{
				const RemapInputSemantic& ris = s_remapInputSemantic[ii];
				if (0 == base::strCmp(ris.m_name, _name)
					&& ris.m_index == _index)
				{
					return ris;
				}
			...
			return s_remapInputSemantic[graphics::Attrib::Count];
		}

		struct UniformRemap
		{
			graphics::UniformType::Enum id;
		...
		{
			{ graphics::UniformType::Sampler, D3D_SVC_SCALAR,         D3D_SVT_INT,         0, 0 },
			{ graphics::UniformType::Vec4, D3D_SVC_VECTOR,         D3D_SVT_FLOAT,       0, 0 },
			{ graphics::UniformType::Mat3, D3D_SVC_MATRIX_COLUMNS, D3D_SVT_FLOAT,       3, 3 },
			{ graphics::UniformType::Mat4, D3D_SVC_MATRIX_COLUMNS, D3D_SVT_FLOAT,       4, 4 },
			{ graphics::UniformType::Sampler, D3D_SVC_OBJECT,         D3D_SVT_SAMPLER,     0, 0 },
			{ graphics::UniformType::Sampler, D3D_SVC_OBJECT,         D3D_SVT_SAMPLER2D,   0, 0 },
			{ graphics::UniformType::Sampler, D3D_SVC_OBJECT,         D3D_SVT_SAMPLER3D,   0, 0 },
			{ graphics::UniformType::Sampler, D3D_SVC_OBJECT,         D3D_SVT_SAMPLERCUBE, 0, 0 },
		};

		graphics::UniformType::Enum findUniformType(const D3D11_SHADER_TYPE_DESC& constDesc)
		{
			for (uint32_t ii = 0; ii < BASE_COUNTOF(s_uniformRemap); ++ii)
			{
		...
		bool getReflectionDataD3D9(ID3DBlob* _code, UniformArray& _uniforms, base::WriterI* _messageWriter)
		{
			...
			while (ptr < end && *ptr != D3DSIO_END)
			{
				...
				if (fourcc == CTAB_CONSTANT)
				{
					...
					{
						base::write(_messageWriter, &messageErr, "Error: Invalid constant table data\n");
						return false;
					}
					break;
				}
			...
			if (!header)
			{
				base::write(_messageWriter, &messageErr, "Error: Could not find constant table data\n");
				return false;
			}
			...
			BASE_TRACE("Creator: %s 0x%08x", creator, header->Version);
			BASE_TRACE("Num constants: %d", header->Constants);
			BASE_TRACE("#   cl ty RxC   S  By Name");
			...
			for (uint32_t ii = 0; ii < header->Constants; ++ii)
			{
				...
				BASE_TRACE("%3d %2d %2d [%dx%d] %d %s[%d] c%d (%d)"
					, ii
					, ctType.Class
					, ctType.Type
					, ctType.Rows
				...
				graphics::UniformType::Enum type = findUniformType(desc);
				if (graphics::UniformType::Count != type)
				{
					Uniform un;
					un.name = '$' == name[0] ? name + 1 : name;
					un.type = isSampler(desc.Type)
						? graphics::UniformType::Enum(kUniformSamplerBit | type)
						: type
						;
		...
		bool getReflectionDataD3D11(ID3DBlob* _code, bool _vshader, UniformArray& _uniforms, uint8_t& _numAttrs, uint16_t* _attrs, uint16_t& _size, UniformNameList& unusedUniforms, base::WriterI* _messageWriter)
		{
			base::Error messageErr;
			...
			if (FAILED(hr))
			{
				base::write(_messageWriter, &messageErr, "Error: D3DReflect failed 0x%08x\n", (uint32_t)hr);
				return false;
			}
			...
			if (FAILED(hr))
			{
				base::write(_messageWriter, &messageErr, "Error: ID3D11ShaderReflection::GetDesc failed 0x%08x\n", (uint32_t)hr);
				return false;
			}

			BASE_TRACE("Creator: %s 0x%08x", desc.Creator, desc.Version);
			BASE_TRACE("Num constant buffers: %d", desc.ConstantBuffers);
			...
			if (_vshader) // Only care about input semantic on vertex shaders
			{
				for (uint32_t ii = 0; ii < desc.InputParameters; ++ii)
				{
					...
					BASE_TRACE("\t%2d: %s%d, vt %d, ct %d, mask %x, reg %d"
						, ii
						, spd.SemanticName
						, spd.SemanticIndex
						, spd.SystemValueType
					...
					if (ris.m_attr != graphics::Attrib::Count)
					{
						_attrs[_numAttrs] = graphics::attribToId(ris.m_attr);
						++_numAttrs;
					}
				}
			}
			...
			for (uint32_t ii = 0; ii < desc.OutputParameters; ++ii)
			{
				...
				BASE_TRACE("\t%2d: %s%d, %d, %d", ii, spd.SemanticName, spd.SemanticIndex, spd.SystemValueType, spd.ComponentType);
			}

			for (uint32_t ii = 0, num = base::uint32_min(1, desc.ConstantBuffers); ii < num; ++ii)
			{
				...
				if (SUCCEEDED(hr))
				{
					...
					for (uint32_t jj = 0; jj < bufferDesc.Variables; ++jj)
					{
						...
						if (SUCCEEDED(hr))
						{
							...
							if (SUCCEEDED(hr))
							{
								graphics::UniformType::Enum uniformType = findUniformType(constDesc);

								if (graphics::UniformType::Count != uniformType
									&& 0 != (varDesc.uFlags & D3D_SVF_USED))
								{
									...
									un.regCount = uint16_t(base::alignUp(varDesc.Size, 16) / 16);
									_uniforms.push_back(un);

									BASE_TRACE("\t%s, %d, size %d, flags 0x%08x, %d (used)"
										, varDesc.Name
										, varDesc.StartOffset
										, varDesc.Size
										, varDesc.uFlags
								...
								else
								{
									...
									BASE_TRACE("\t%s, unknown type", varDesc.Name);
								}
							}
						}
					}
			...
			for (uint32_t ii = 0; ii < desc.BoundResources; ++ii)
			{
				...
				if (SUCCEEDED(hr))
				{
					if (D3D_SIT_SAMPLER == bindDesc.Type || D3D_SIT_TEXTURE == bindDesc.Type)
					{
						...
						base::StringView end = base::strFind(bindDesc.Name, "Sampler");
						if (end.isEmpty())
							end = base::strFind(bindDesc.Name, "Texture");

						if (!end.isEmpty())
						{
							Uniform un;
							un.name.assign(bindDesc.Name, (end.getPtr() - bindDesc.Name));
							un.type = graphics::UniformType::Enum(kUniformSamplerBit | graphics::UniformType::Sampler);
							un.num = 1;
					...
					{
						BASE_TRACE("\t%s, unknown bind data", bindDesc.Name);
					}
				}
			}
		...
		static bool compile(const Options& _options, uint32_t _version, const std::string& _code, base::WriterI* _shaderWriter, base::WriterI* _messageWriter, bool _firstPass)
		{
			...
			if (profile[0] == '\0')
			{
				base::write(_messageWriter, &messageErr, "Error: Shader profile must be specified.\n");
				return false;
			}

			char profileAndType[8] = {};
			profileAndType[0] = (_options.shaderType == 'f') ? 'p' : _options.shaderType;
			base::strCat(profileAndType, BASE_COUNTOF(profileAndType), profile);
			...
			{
				uint32_t optimization = base::uint32_min(_options.optimizationLevel, BASE_COUNTOF(s_optimizationLevelD3D11) - 1);
			...
			BASE_TRACE("Profile: %s", profile);
			BASE_TRACE("Flags: 0x%08x", flags);
				...
				|| (werror && NULL != errorMsg))
			{
				...
				if (!hlslfp.empty())
				{
					base::StringView logfp = base::strFind(log, hlslfp.c_str());
					if (!logfp.isEmpty())
					{
				...
				base::write(_messageWriter, &messageErr, "Error: D3DCompile failed 0x%08x %s\n", (uint32_t)hr, log);
				errorMsg->Release();
				return false;
			}
			...
			uint8_t numAttrs = 0;
			uint16_t attrs[graphics::Attrib::Count];
			uint16_t size = 0;
			...
			if (_version < 400)
			{
				...
				{
					base::write(_messageWriter, &messageErr, "Error: Unable to get D3D9 reflection data.\n");
					goto error;
				}
			}
			else
			{
				...
				{
					base::write(_messageWriter, &messageErr, "Error: Unable to get D3D11 reflection data.\n");
					goto error;
				}
					...
					&& unusedUniforms.size() > 0)
				{
					...
					while (!reader.isDone())
					{
						...
						for (UniformNameList::iterator it = unusedUniforms.begin(), itEnd = unusedUniforms.end(); it != itEnd; ++it)
						{
							...
							if (!base::findIdentifierMatch(strLine, it->c_str()).isEmpty())
							{
...

			{
				...
				for (UniformArray::const_iterator it = uniforms.begin(); it != uniforms.end(); ++it)
				{
					...
					base::write(_shaderWriter, type, &err);
					...
					BASE_TRACE("%s, %s, %d, %d, %d"
						, un.name.c_str()
						, getUniformTypeName(graphics::UniformType::Enum(un.type & ~kUniformMask))
						, un.num
						, un.regIndex
						, un.regCount
					);
...

			{
				...
				base::write(_shaderWriter, code->GetBufferPointer(), shaderSize, &err);
				uint8_t nul = 0;
			...
			if (_version >= 400)
			{
				base::write(_shaderWriter, numAttrs, &err);
				base::write(_shaderWriter, attrs, numAttrs * sizeof(uint16_t), &err);
	...
	bool compileHLSLShader(const Options& _options, uint32_t _version, const std::string& _code, base::WriterI* _shaderWriter, base::WriterI* _messageWriter)
	{
...
namespace bgfx
{
	bool compileHLSLShader(const Options& _options, uint32_t _version, const std::string& _code, base::WriterI* _shaderWriter, base::WriterI* _messageWriter)
	{
		...
		base::Error messageErr;
		base::write(_messageWriter, &messageErr, "HLSL compiler is not supported on this platform.\n");
		return false;
	}
