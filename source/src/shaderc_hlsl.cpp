...
namespace shaderc { namespace hlsl
{
	...
	{
		{ bgfx::UniformType::Sampler, D3D_SVC_SCALAR,         D3D_SVT_INT,         0, 0 },
		{ bgfx::UniformType::Vec4, D3D_SVC_VECTOR,         D3D_SVT_FLOAT,       0, 0 },
		{ bgfx::UniformType::Mat3, D3D_SVC_MATRIX_COLUMNS, D3D_SVT_FLOAT,       3, 3 },
		{ bgfx::UniformType::Mat4, D3D_SVC_MATRIX_COLUMNS, D3D_SVT_FLOAT,       4, 4 },
		{ bgfx::UniformType::Sampler, D3D_SVC_OBJECT,         D3D_SVT_SAMPLER,     0, 0 },
		{ bgfx::UniformType::Sampler, D3D_SVC_OBJECT,         D3D_SVT_SAMPLER2D,   0, 0 },
		{ bgfx::UniformType::Sampler, D3D_SVC_OBJECT,         D3D_SVT_SAMPLER3D,   0, 0 },
		{ bgfx::UniformType::Sampler, D3D_SVC_OBJECT,         D3D_SVT_SAMPLERCUBE, 0, 0 },
	};

	bgfx::UniformType::Enum findUniformType(const D3D11_SHADER_TYPE_DESC& constDesc)
	{
	...
	bool getReflectionDataD3D9(ID3DBlob* _code, UniformArray& _uniforms, bx::WriterI* _messageWriter)
	{
		...
		for (uint32_t ii = 0; ii < header->Constants; ++ii)
		{
			...
			bgfx::UniformType::Enum type = findUniformType(desc);
			if (bgfx::UniformType::Count != type)
			{
				Uniform un;
				un.name = '$' == name[0] ? name + 1 : name;
				un.type = isSampler(desc.Type)
					? bgfx::UniformType::Enum(kUniformSamplerBit | type)
					: type
					;
			...
			if (SUCCEEDED(hr) )
			{
				...
				for (uint32_t jj = 0; jj < bufferDesc.Variables; ++jj)
				{
					...
					if (SUCCEEDED(hr) )
					{
						...
						if (SUCCEEDED(hr) )
						{
							bgfx::UniformType::Enum uniformType = findUniformType(constDesc);

							if (bgfx::UniformType::Count != uniformType
			...
			if (SUCCEEDED(hr) )
			{
				if (D3D_SIT_SAMPLER == bindDesc.Type || D3D_SIT_TEXTURE == bindDesc.Type)
				{
					...
					if (!end.isEmpty() )
					{
						...
						un.type = bgfx::UniformType::Enum(kUniformSamplerBit | bgfx::UniformType::Sampler);
						un.num = 1;
	...
	static bool compile(const Options& _options, uint32_t _version, const std::string& _code, bx::WriterI* _shaderWriter, bx::WriterI* _messageWriter, bool _firstPass)
	{
...

		{
			...
			for (UniformArray::const_iterator it = uniforms.begin(); it != uniforms.end(); ++it)
			{
				...
				BX_TRACE("%s, %s, %d, %d, %d"
					, un.name.c_str()
					, getUniformTypeName(bgfx::UniformType::Enum(un.type & ~kUniformMask))
					, un.num
					, un.regIndex
					, un.regCount
					);
